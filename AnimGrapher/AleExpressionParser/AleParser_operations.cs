    ...
    public partial class AleExpressionParser : AleSimpleLexer
    {
        ...
        protected bool BuiltinDivOperation(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            ...
            switch (optype)
            {
                ...
                case TypeCode.UInt64:
                    //result.Value = Convert.ToUInt64(a.Value) / Convert.ToUInt64(b.Value);
                    result.Value = Convert.ToUInt64(a.Value) / Convert.ToDouble(b.Value);
                    return true;
                case TypeCode.Int64:
                    //result.Value = Convert.ToInt64(a.Value) / Convert.ToInt64(b.Value);
                    result.Value = Convert.ToInt64(a.Value) / Convert.ToDouble(b.Value);
                    return true;
                case TypeCode.UInt32:
                    //result.Value = Convert.ToUInt32(a.Value) / Convert.ToUInt32(b.Value);
                    result.Value = Convert.ToUInt32(a.Value) / Convert.ToDouble(b.Value);
                    return true;
                case TypeCode.Int32:
                    //result.Value = Convert.ToInt32(a.Value) / Convert.ToInt32(b.Value);
                    result.Value = Convert.ToInt32(a.Value) / Convert.ToDouble(b.Value);
                    return true;
                default:
                    result.SetError(AleTermResult.ERROR_INCOMPATIBLETYPES, term.Token.StartInOrigin);
                    return false;
        ...
        protected bool BuiltinPowOperation(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            AleTerm t1 = term[0];
            AleTerm t2 = term[1];
            ...
            switch (optype)
            {
                case TypeCode.Double:
                    ...
                    return true;
                case TypeCode.Decimal:
                    result.Value = Convert.ToDecimal(Math.Pow(Convert.ToDouble(a.Value), Convert.ToDouble(b.Value)));
                    return true;
                case TypeCode.UInt64:
                    result.Value = Math.Pow(Convert.ToUInt64(a.Value), Convert.ToUInt64(b.Value));
                    return true;
                case TypeCode.Int64:
                    result.Value = Math.Pow(Convert.ToInt64(a.Value), Convert.ToInt64(b.Value));
                    return true;
                case TypeCode.UInt32:
                    result.Value = Math.Pow(Convert.ToUInt32(a.Value), Convert.ToUInt32(b.Value));
                    return true;
                case TypeCode.Int32:
                    result.Value = Math.Pow(Convert.ToInt32(a.Value), Convert.ToInt32(b.Value));
                    return true;
                default:
                    result.SetError(AleTermResult.ERROR_INCOMPATIBLETYPES, term.Token.StartInOrigin);
                    return false;
        ...
        protected bool BuiltinSin2Function(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Sin(Convert.ToDouble(parameters.FirstParam)) * Math.Sin(Convert.ToDouble(parameters.FirstParam));
            return true;
        }

        protected bool BuiltinCos2Function(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Cos(Convert.ToDouble(parameters.FirstParam)) * Math.Cos(Convert.ToDouble(parameters.FirstParam));
            return true;
        }

        protected bool BuiltinTan2Function(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Tan(Convert.ToDouble(parameters.FirstParam)) * Math.Tan(Convert.ToDouble(parameters.FirstParam));
            return true;
        }
        ...
        protected bool BuiltinLog2Function(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Log(Convert.ToDouble(parameters.FirstParam))/Math.Log(2);
            return true;
        }
        ...
        protected bool BuiltinFracFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            double firstParam = Convert.ToDouble(parameters.FirstParam);
            result.Value = Math.Abs(firstParam - Math.Truncate(firstParam));
            return true;
        }

        protected bool BuiltinSignFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            double firstParam = Convert.ToDouble(parameters.FirstParam);
            //if (firstParam > 0)
            //    result.Value = 1;
            ...
            //else
            //    result.Value = 0;
            result.Value = Math.Sign(firstParam);
            return true;
        }

        protected bool BuiltinHeavysideFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            double firstParam = Convert.ToDouble(parameters.FirstParam);
            if (firstParam > 0)
                result.Value = 1;
        ...
        protected bool BuiltinFibonacciFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            AleTerm t1 = term[0];
            ...
            double n = Convert.ToInt32(a.Value);
            if (n < 0 && result.SetError(AleTermResult.ERROR_EVALUATION, term.Token.StartInOrigin)) return false;
        ...
        protected bool BuiltinIsPrimeFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            AleTerm t1 = term[0];
        ...
        protected bool BuiltinMinFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Min(Convert.ToDouble(parameters.FirstParam), Convert.ToDouble(parameters.SecondParam));
            return true;
        }

        protected bool BuiltinMaxFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            result.Value = Math.Max(Convert.ToDouble(parameters.FirstParam), Convert.ToDouble(parameters.SecondParam));
            return true;
        }

        protected bool BuiltinGCDFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            AleTerm t1 = term[0];
            AleTerm t2 = term[1];
            ...
            double nDouble = Convert.ToDouble(a.Value);
            double mDouble = Convert.ToDouble(b.Value);
            if (nDouble != Math.Floor(nDouble) || mDouble != Math.Floor(mDouble))
            {
                result.SetError(AleTermResult.ERROR_INCOMPATIBLETYPES, term.Token.StartInOrigin);
                return false;
            }

            // check if allowed values
            int n = Convert.ToInt32(a.Value);
            int m = Convert.ToInt32(b.Value);
            if (n < 0 && result.SetError(AleTermResult.ERROR_EVALUATION, term.Token.StartInOrigin)) return false;
            if (m < 0 && result.SetError(AleTermResult.ERROR_EVALUATION, term.Token.StartInOrigin)) return false;
            ...
            result.Value = (n == 0) ? m : n;

            return true;
        }

        protected bool BuiltinLCMFunction(AleTerm term, ref OperationEvalParameters parameters, ref AleTermResult result)
        {
            AleTerm t1 = term[0];
            AleTerm t2 = term[1];
            ...
            int gcd = (n_rec == 0) ? m_rec : n_rec;

            // lcm = n*m / gcd
            result.Value = (n * m) / gcd;
        ...
        protected void InitStandardOperations()
        {
            ...
            // "^" operator
            AddOperation(new AleOperation("^", 3000, AleOperation.OPERATOR_YFX)
            {
                Evaluator = BuiltinPowOperation
            });
        ...
        public void InitOperations(int opset, bool addStdFunctions = true, bool clear = true)
        {
            ...
            if (addStdFunctions)
            {
                ...
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinSin2Function
                });

                AddOperation(new AleOperation(IgnoreCase ? "COS²" : "Cos²")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinCos2Function
                });

                AddOperation(new AleOperation(IgnoreCase ? "TAN²" : "Tan²")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinTan2Function
                });
                ...
                AddOperation(new AleOperation(IgnoreCase ? "LN" : "Ln")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                ...
                AddOperation(new AleOperation(IgnoreCase ? "LOG2" : "Log2")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinLog2Function
                });
                ...
                AddOperation(new AleOperation(IgnoreCase ? "FRAC" : "Frac")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinFracFunction
                });
                ...
                AddOperation(new AleOperation(IgnoreCase ? "H" : "H")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinHeavysideFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "FIBO" : "Fibo")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinFibonacciFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "ISPRIME" : "Isprime")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinIsPrimeFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "MIN" : "Min")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null), new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinMinFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "MAX" : "Max")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null), new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinMaxFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "GCD" : "Gcd")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null), new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinGCDFunction
                });

                AddOperation(new AleOperation(IgnoreCase ? "LCM" : "Lcm")
                {
                    Parameters = new List<Tuple<TypeCode, object>>() { new Tuple<TypeCode, object>(TypeCode.Double, null), new Tuple<TypeCode, object>(TypeCode.Double, null) },
                    Evaluator = BuiltinLCMFunction
                });
